...
# !!!!!!!!!!!!!!!!! READ TSP POINTS HERE !!!!!!!!!!!!!!!!!!!!###########################################################
def read_file_fence():
    global Fence_Points
    with open('berlin52.txt') as f:
        for line in f:  # read rest of lines
            Fence_Points.append([float(i) for i in line.split()])
...
def recalculate_qp():
    ...
    quadrant_points = []
    for i in range(4):
        best = x.index(max(x)) if i == 0 else y.index(min(y)) if i == 1 else x.index(min(x)) if i == 2 else y.index(max(y))
        tmp_qp = []
        for pp in range(len(x)):
            if Fence_Points[pp][i%2] == Fence_Points[best][i%2]:
                tmp_qp.append(pp)
...
def get_next_perimeter():
    ...
    if len(Fence_Points) <= 3:
        ret = [i for i in range(len(Fence_Points))]
        Fence_Points = []
        return ret

    x = [Fence_Points[i][0] for i in range(len(Fence_Points))]
    y = [Fence_Points[i][1] for i in range(len(Fence_Points))]
    recalculate_qp()
    current_quadrant = 0
    perimeter = []
    perimeter += quadrant_points[0]
    key_points = [i[0] for i in quadrant_points]

    while perimeter[0] != perimeter[-1] or len(perimeter) == 1:
        current_point = perimeter[-1]

        if current_quadrant == 0:
            possible_points = [i for i in range(len(x)) if y[i] < y[current_point] and x[i] < x[current_point]]
...
Fence_Points = []
read_file_fence()
# perimeter = get_next_perimeter()
# plt.plot([x[i] for i in perimeter], [y[i] for i in perimeter], 'bo', linestyle="solid", linewidth=2)
####################NEW_FIGURE_ALGORITHM################################################################################
...
def matrix_get_minimum_distance():
    ...
    for i_row in range(len(distance_matrix)):
        for i_point in range(len(Points)):
            if distance_matrix[i_row][i_point] == -1:
                ...
                tmp = distance_matrix[i_row][i_point]
                if tmp == min:
                    new_case = True
    ...
    for case in range(len(answer)):
        ...
        tmp = dist(potential[0],Figure[potential[1]][0]) + dist(potential[0],Figure[potential[1]][1])
        if tmp < best_perimeter:
            best_perimeter = tmp
            best_case = case
...
def init_first_triangle(init_points):
    ...
    for i in range(len(init_points)):
        Figure.append([init_points[i], init_points[i+1 if i+1 != len(init_points) else 0]])
...
def animate():
    ...
    my.append(my[0])
    plt.cla()
    plt.plot(xx, yy, 'go', linestyle=":", linewidth=1)
...
#perim = get_next_perimeter()[:-1]
#init_first_triangle(perim)
