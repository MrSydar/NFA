...
def read_points(destination, filePath):
    """
    Read points from file.
    File format: x y \n x y \n ... x y
    :param destination: 2D array to fill.
                        array format [[ x, y], ..., [ x, y]]
    :param filePath: path to file with points coordinates
    """
    with open(filePath) as f:
        for line in f:
            destination.append([float(it) for it in line.split()])

def recalculate_qp(x, y):
    quadrant_points = []

    for it in range(4):
        tmp_qp = []
        best = x.index(max(x)) if it == 0 else \
               y.index(min(y)) if it == 1 else \
               x.index(min(x)) if it == 2 else \
               y.index(max(y))
...
def get_next_perimeter():
    ...
    if len(Fence_Points) <= 3:
        Fence_Points = []
        return [idx for idx in range(len(Fence_Points))], [], []

    x = [Fence_Points[it][0] for it in range(len(Fence_Points))]
    y = [Fence_Points[it][1] for it in range(len(Fence_Points))]

    current_quadrant = 0
    quadrant_points = recalculate_qp(x, y)
    perimeter = deepcopy(quadrant_points[0])
...
def matrix_get_minimum_distance():
    ...
    for i_row in range(len(distance_matrix)):
        for i_point in range(len(Points)):
            if distance_matrix[i_row][i_point] == -1:
                ...
                tmp2 = distance_matrix[i_row][i_point]
                if tmp2 == min:
                    new_case = True
    ...
    for case in range(len(answer)):
        ...
        tmp2 = dist(potential[0], Figure[potential[1]][0]) + dist(potential[0], Figure[potential[1]][1])
        if tmp2 < best_perimeter:
            best_perimeter = tmp2
            best_case = case
...
def init_first_triangle(init_points):
...
def animate():
    mx = [Points[x[0]][0] for x in Figure]
    mx.append(mx[0])
    my = [Points[y[0]][1] for y in Figure]
    my.append(my[0])
    plt.cla()
    plt.plot(xx, yy, 'bo')
...
def translate_perimeter(args):
    ...
    translated = []
    for per in my_perim:
        for pppp1 in range(len(Points)):
            if Points[pppp1][0] == x[per] and Points[pppp1][1] == y[per]:
                translated.append(pppp1)

    return translated


Fence_Points, Figure, distance_matrix, PL = [], [], [], []
read_points(Fence_Points, 'C:\\Users\\Den\\Desktop\\Study\\3й сем\\berlin52.txt')

Points = deepcopy(Fence_Points)
xx = [cord[0] for cord in Points]
xx.append(xx[0])
yy = [cord[1] for cord in Points]
yy.append(yy[0])
Points_Visited = [True] * len(Points)

perim = translate_perimeter(get_next_perimeter())
while len(perim):
    PL.append(perim)
    perim = list(set(translate_perimeter(get_next_perimeter())))
...
# plt.plot([Points[i[0]][0] for i in Figure], [Points[i[0]][1] for i in Figure], 'mo', linestyle="solid", linewidth=1)
plt.show()
#######################################END
####################NEW_FIGURE_ALGORITHM_END############################################################################
